...
open class TestJobService : JobService() {
    companion object {
        const val TAG = "Job"
    ...
    private val mJobHandler = Handler() {
        ...
        if (it.what == DELEY_MESSAGE_ID) {
            ...
            Log.i(TAG, "handle message: JOB_CYCLE_ID start static service")
            startStaticService()
        }
    ...
    override fun onStartJob(params: JobParameters?): Boolean {
        ...
        if (params?.jobId == Constants.JOB_APP_START_ID) {
            ...
            Log.i(TAG, "JOB_CYCLE_ID onStartJob: Handler delay 6s start static service")

            if (ServiceUtils.isServiceRunning(StaticService::class.java)) {
                Log.i(TAG, "JOB_CYCLE_ID onStartJob: Static Service is running and stop")
    ...
    private fun startStaticService() {
        //启动前台服务然后启动job
        ...
        Log.i(TAG, "onStartJob: start static service")
    }
